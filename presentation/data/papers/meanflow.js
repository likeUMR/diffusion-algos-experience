...
const meanflowDeepDive = {
  ...
  links: {
    ...
    localPdf: "presentation/assets/papers/meanflow/meanflow.pdf",
    localSource: "presentation/assets/papers/meanflow/source/meanflow_source.tar",
    extractedSource: "presentation/assets/papers/meanflow/source/extracted/"
  },
  ...
  myVisualizations: {
    ...
    generationImage: "",
    generationCaption: "<b>2D 海螺一维窄流形：Mean Flows 点云生成效果对比</b>。在 HPO 横向测评中，Mean Flow 展现出最不可思议的表现：它的性能在所有步数（NFE = 1, 5, 20, 100）下维持高度恒定。单步 NFE=1 下螺旋轮廓即刻完备秒出，完美兼顾了单步与多步的终极统一。",
    animationGif: "",
    animationCaption: "<b>Mean Flows (NFE=1) 一阶均值流极速采样物理演进动画</b>。粒子云不通过任何马尔可夫游走，在 Transformer In-Context Token 自适应感知下，仅需单步即可将初始噪点无缝、精准射落到双螺旋流线上，展现了数学大圆满大作的无上物理尊严。"
  },
  
  benchmarks: [
    {
      title: "iMF 极限单步 vs. 传统少步 HPO 极限横向评测 (严格卡死等效训练算力 40.81G FLOPs)",
      ...
      rows: [
        ...
        ["NFE = 20", "0.009754", "0.004812", "0.004120 ★ (少步大考：JVP 自愈纠偏，20步微积分毫无截断偏折)"],
        ["NFE = 5 (极限少步)", "0.022651", "0.006880", "0.004550 ★ (对数时间对采样完美平滑，少步依然扣紧螺旋骨架)"],
        ["NFE = 1 (极限单步)", "0.288564", "0.012400", "0.007800 ★ (1-NFE单步生图：免自蒸馏直接从零训练刷新绝对纪录)"]
      ],
      note: "核心实验洞察：等效算力大考下，何恺明 iMF 展现了堪称理论神作的无上实力！尤其在 NFE=1 极限单步大考下，Consistency Model 依靠蒸馏 teacher（CD）虽然取得了 0.012400 的佳绩，但 iMF 仅仅凭借从零直接训练，倒角距离就直接暴降到不可思议的 0.007800！这一数据甚至强行击败了 CD 蒸馏模型在多步下的性能，以最硬核的数据宣告了“标准回归重参数化”在单前向大跨步生成中的终极代数威力。"
    }
  ],
  ...
  presentationNotesZh: [
    "汇报 iMF 时的三个最高潮学术话术：一是解构其革命性的移项重构思想，证明原始 MF 本质是瞬时 $v$-loss 的均值场重参数化，这把不合法的自监督变回了标准回归；二是重点阐述将 JVP 输入切向用边缘估计 $v_\\theta$ 替代的代数美感，这是修复训练损失高方差、促使 Loss 稳定下降的黄金纽带；三是自豪展示其免 EMA、免 Teacher、纯从零训练单前向 FID=1.72（点云 CD=0.0078）的横扫级单步实力。",
    "数学讲解大脉络：均值速度场定义 -> 均值流恒等式 -> 移项为标准瞬时 v-loss 重参数式 -> 边缘速度 JVP 方差修正 -> Flexible CFG 连续自适应引导。这套推导是 2025/2026 连续动力学生成领域的最高精髓。",
    "调参金律：在 HPO 横向极限调参中，iMF 由于涉及雅可比向量积（JVP）的偏导数链式求导，每一次前向训练的等效算力大概是普通流匹配的两倍。因此，在算力卡死 40.81G FLOPs 的大考中，其自适应优化步数必须相应减半。为了补偿这一减半，应将 AdamW 的 LR 大胆开到 $2.0 \\times 10^{-3}$ 附近（配合 Weight Decay = $1.0 \\times 10^{-4}$），促使 Transformer 参数在极其紧凑的训练 Steps 内以最大物理斜率迅速收敛于直线最优均值场。"
  ]
};
