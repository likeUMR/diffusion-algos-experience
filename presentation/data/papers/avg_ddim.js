...
const avgDdimDeepDive = {
  ...
  mathNarrative: {
    ...
    modules: [
      ...
      },
      {
        ...
        description: "为了构造方差更低的回归目标，Avg-DDIM 引入了一个大小为 $k$ 的局部潜在候选集 $\\mathcal{N}_k(x_t)$。对于这 $k$ 个候选解释，由于前向加噪过程 $q(\\mathbf{x}_t|\\mathbf{x}_0)$ 符合各向同性高斯分布，我们可以通过指数 Softmax 变换，解析地计算各个候选样本对于当前带噪状态的相对权重，从而构造条件期望噪声目标 $\\bar{\\epsilon}$。",
        formulas: [
          {
            name: "候选解释集与前向概率加权",
  ...
  originalPaperBenchmarks: [
    {
      ...
      note: "Avg-DDIM 是我在 2D 窄流形实验中针对大步长 ODE 积分抖动和跨分支偏折问题尝试的一个改进 idea。它通过在切向引入局域 K-Nearest Neighbors 期望滤波，降低高方差噪声目标的影响，并在当前 HPO 设置中表现出较好的收敛性。"
    }
  ],
  ...
  presentationNotesZh: [
    "在汇报 Avg-DDIM 时的三向学术故事线：一是提出“窄流形多解拉扯”这一扩散模型在特定几何设置下可能出现的问题，指出 minibatch 之间的方向冲突可能导致去噪场粗糙胖化；二是提出“期望代替单向”的设计思路，利用 q(xt|x0) 高斯前向闭式实现低成本的 Softmax 加权期望合成；三是在算力等效约束下，展示其在 NFE=100 上取得 0.004435 的当前最优结果，同时强调仍需多 seed 验证。",
    "数学讲解大脉络：单配对 $L_{DDIM}$ 冲突缺陷 -> 局部后验候选集 $\\mathcal{N}_k$ -> 前向高斯似然 Softmax 解析加权 $w_i$ -> 局域高斯拒绝采样门控接受概率。在组会幻灯片上，可以把“梳齿状震荡向量场”与经过期望平滑后的“低通光滑平铺向量场”做两个侧向对比图。",
    "调参经验：Avg-DDIM 对局域半径超参 $\\sigma_{local}$ 较敏感。如果 $\\sigma_{local}$ 设定得过大（如 $\\sigma_{local} > 1.0$），会将螺旋线远端不相干分支的粒子也纳入加权平均，引入跨分支偏置并导致 CD 指标恶化；当前 HPO 找到的较优半径为 $\\sigma_{local}=0.3$，候选集更集中在当前粒子所在的局部邻域内。"
  ]
};
